...
def run_vespa(fasta_file,cache_path):
    ...
    vespa_out = os.path.join(cwd,'vespa_run_directory','output')
    return vespa_out
...
def run_mmseq(gen_fasta,ref_fasta,gen_name,threads):
    ...
    mmseq_out = os.path.join(cwd,gen_name+'_aln.tab')
    return mmseq_out
...
def generate_msa(ref_fasta,gen_fasta,threads):
    ...
    ref_gen_msa = os.path.join('vespa_msa','ref_gen_msa.fasta')
    return ref_gen_msa
    ...
    if remainder_threads == 0:
        ...
        with open ('vespa_pair_cmd.list','w') as cmd:
            for i in range(len(gen_dict)):
                ...
                gen_seq = gen_dict[gen_id]
                cmd.write('python vespa_pair.py --mmseq_out '+mmseq_out+' --vespa_out '+vespa_out+'/'+str(i)+'.csv --ref_id '+ref_id+' --ref_seq '+ref_seq+' --gen_id '+gen_id+' --gen_seq '+gen_seq+'\n')
        with open ('vespa_pair_cmd_remainder.list','w') as cmd:
            for i in range(len(gen_dict)-remainder_threads,len(gen_dict)):
                ref_id = list(ref_dict.keys())[i]
                ref_seq = ref_dict[ref_id]
                gen_id = list(gen_dict.keys())[i]
                gen_seq = gen_dict[gen_id]
                cmd.write('python map_score.py --mmseq_out '+mmseq_out+' --vespa_out '+vespa_out+'/'+str(i)+'.csv --ref_id '+ref_id+' --ref_seq '+ref_seq+' --gen_id '+gen_id+' --gen_seq '+gen_seq+'\n')
        os.system('parallel -j '+str(threads)+' < vespa_pair_cmd.list')
        os.system('parallel -j '+str(remainder_threads)+' < vespa_pair_cmd_remainder.list')
...
def msa_map(ref_fasta,gen_fasta,threads):
    ...
    cons_fasta = os.path.join(cwd,'vespa_run_directory','conspred_class.fasta')
    cons_dict = fasta2dict(cons_fasta)
    mapped_scores = os.path.join(cwd,'vespa_scores')
    if remainder_threads == 0:
        with open('vespa_msa_cmd.list','w') as cmd:
            for i in range(len(gen_dict)):
                ...
                vespa_map_out = os.path.join(cwd,'vespa_scores',gen_id+'_vespa_scores.csv')
        ...
        with open('vespa_msa_cmd.list','w') as cmd:
            for i in range(len(gen_dict)):
                ...
                vespa_map_out = os.path.join(cwd,'vespa_scores',gen_id+'_vespa_scores.csv')
                cmd.write('python vespa_msa.py --vespa_map_out '+vespa_map_out+' --ref_id '+ref_id+' --ref_aln '+ref_aln.replace('-','_')+' --cons_class '+cons_class+' --gen_id '+gen_id+' --gen_aln "'+gen_aln.replace('-','_')+'"\n')
        with open ('vespa_msa_cmd_remainder.list','w') as cmd:
            for i in range(len(gen_dict)-remainder_threads,len(gen_dict)):
                ref_id = list(ref_dict.keys())[i]
                gen_id = list(gen_dict.keys())[i]
                ref_aln = msa_dict[ref_id]
                ...
                cmd.write('python vespa_msa.py --vespa_map_out '+vespa_map_out+' --ref_id '+ref_id+' --ref_aln '+ref_aln.replace('-','_')+' --cons_class '+cons_class+' --gen_id '+gen_id+' --gen_aln "'+gen_aln.replace('-','_')+'"\n')
        os.system('parallel -j '+str(threads)+' < vespa_msa_cmd.list')   
        os.system('parallel -j '+str(remainder_threads)+' < vespa_msa_cmd_remainder.list')   
...
def parse_input(args):
    ...
    return [ref_fasta,ref_name,gen_fasta,gen_name,cache_path,threads]

def optmize_threads(threads):
    '''
    This function optmizes the number of threads,
    since the *Parallel* package requires that the number of jobs/sequences
    to run in parallel should be divisble by the number of the specified threads
    '''
    remainder_thread = 0
    if len(gen_dict)%threads != 0:
        for i in range(len(gen_dict)):
            if (len(gen_dict)-i)%threads == 0:
                remainder_thread = i
                break
    return remainder_thread
